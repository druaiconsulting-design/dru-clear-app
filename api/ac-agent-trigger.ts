...
import { GENIUS_MODE, VOICE_DNA, DIRECT_RESPONSE_PHILOSOPHY, getAgentKnowledge, getAgentCorrections } from './_lib/agentKnowledge.js';
...
async function callAnthropic(prompt: string, maxTokens = 2000, model = 'claude-haiku-4-5-20251001'): Promise<string> {
  ...
  console.log(`[${model}] stop_reason: ${data.stop_reason ?? 'unknown'}`);
...
async function runDarius(): Promise<string|null> {
  ...
  // critique hook -- this was the direct cause of the Sept 8, 2026 negative,
  // generic post ("Most leaders treat clarity... Wrong.") and matches
