...
async function runKwameProspectScout(): Promise<{count:number;csqId:string|null}> {
  try {
    ...
    const prompt = `${GENIUS_MODE}\n\n${agentKnowledge}\n\n${VOICE_DNA}${agentCorrections}\n\nYou are Kwame Asante, Prospect Scout for DRU AI Consulting — DeAnna R. Upshaw, AI Authority. Her positioning is "${positioning}."\n\nSearch the web for real, current signals of leaders and organizations showing they need what she offers — public posts, articles, interviews, or news where someone expresses a leadership, culture, or AI-adoption pain point. Signal-only: do NOT filter or select by title, company size, industry, or demographic. Any real signal of the right pain point qualifies, regardless of who it comes from.\n\nHARD REQUIREMENT — REAL NAMED INDIVIDUAL ONLY: prospect_name must be one real, specific, named human being (or organization must be one real, specific, named company) directly tied to a real quote, post, article, or interview you actually found. NEVER invent a demographic label, segment, "type" of leader, or statistic-based category as a substitute (e.g. "CFO/Finance Leadership Segment," "Enterprise organizations across sectors," "C-Suite Leadership Segment (Generic Signal)" are ALL INVALID — these are fabrications, not prospects). A trend report or stats article about a broad group is NOT a prospect and must be discarded, even if it's thematically on point. If a search only returns aggregate/industry-report content with no real named individual attached, that search contributes zero opportunities — do not manufacture one to fill the gap.\n\nIDENTITY-LEVEL STAKE, NOT TOPIC-LEVEL: do not count it as a signal just because someone mentioned AI, leadership, or culture as a topic. The signal must reveal what the struggle actually costs THIS PERSON personally — their credibility, their team's trust in them, their sense of control, their relevance — not a neutral observation about the industry. "Our company is exploring AI adoption" is topic-level and does not qualify. "I no longer trust my own judgment in exec meetings since we started rolling out AI" is identity-level and does qualify. Hold every candidate to this bar before including it.\n\nRespond with ONLY a single JSON object, no preamble, no markdown fences:\n{\n  \"opportunities\": [\n    {\n      \"prospect_name\": string (one real named individual — never a segment, category, or demographic label),\n      \"organization\": string (one real named company — never an industry or sector description),\n      \"stake_word\": string (ONE word naming the personal, identity-level stake this reveals — e.g. "Credibility," "Control," "Relevance," "Trust" — if you cannot name one honest word, this is not a real prospect; discard it),\n      \"signal_summary\": string (1-2 sentences on the real signal found, naming the person and what it personally cost them),\n      \"matched_question\": string (one open-ended, declarative question in DeAnna's voice that speaks directly to this signal — follow the HOOK/QUESTION RULE above exactly: never yes/no, never \"X or Y\"),\n      \"source_url\": string,\n      \"fit_score\": number (1-10),\n      \"fit_reasoning\": string (1-2 sentences),\n      \"status\": \"new\"\n    }\n  ]\n}\nOnly include prospects that pass BOTH hard requirements above with real, current information you actually found. A genuine zero-result day is a normal, expected outcome — if nothing clears both bars, return {\"opportunities\": []}. Returning zero is always correct over inventing a category.`;
    ...
    const top = newOnes.slice(0,5).map((o:any) => [
      ...
      `Stake: ${o.stake_word ?? 'n/a'} | Fit: ${o.fit_score}/10`,
