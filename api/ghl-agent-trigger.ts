...
// Aaliyah CC Outreach \u2014 fires GHL webhook based on upsell signal from CC agents
...
// Command Chain — Command Layer (PARALLELIZED — fixes 60s timeout)
async function runCommandLayer(): Promise<{reviewed:number}> {
  const items=await getCSQItems('governance_cleared');
  console.log(`[command_layer] Reviewing ${items.length} governance-cleared items (fully parallel)...`);
  if (items.length===0) return {reviewed:0};

  await Promise.all(items.map(async(item)=>{
    try {
      // Raymond, Travis, Priya all run simultaneously per item
...
export default async function handler(req:any,res:any): Promise<void> {
  ...
  // CC Upsell Signal \u2014 Aaliyah routing (triggered by cc-agent-trigger.ts upsell signals)
