...
export default async function handler(req:any,res:any): Promise<void> {
  ...
  if (route.pipeline==='p1_omar'){const omar=await runOmar();const ryan=await runRyan(omar);res.status(202).json({success:true,agent:route.agent_name,leads_scanned:omar.total_leads_scanned,high_intent:omar.high_intent_leads.length,crm_updates:ryan.crm_updates});}
    ...
    const id=await runAgentToCSQ('yara','Yara Mansour','Content & Brand','spanish_localization','localization',`You are Yara Mansour, Translator for DRU AI Consulting — DeAnna R. Upshaw, AI Authority. ${topPost?`Translate and localize the following post for LATAM executives. Output BOTH versions so DeAnna can review the English before posting the Spanish.\n\n## ENGLISH VERSION (for review):\n${topPost}\n\n## SPANISH VERSION (ready to post):\n[Full Spanish translation — natural, executive-level LATAM Spanish, not literal]\n\nLocalization notes: [brief notes on any cultural adaptations]\nTranslated hashtags: [Spanish equivalents]\nKeep assessment.druaiconsulting.com as the CTA in both versions.`:'Write an original LinkedIn post for LATAM executives on AI leadership. Output BOTH versions:\n\n## ENGLISH VERSION (for review):\n[English original — 150-250 words, strong hook, one DRU framework reference, CTA to assessment.druaiconsulting.com]\n\n## SPANISH VERSION (ready to post):\n[Full Spanish translation — natural executive-level LATAM Spanish]\n\nTranslated hashtags: [Spanish equivalents]\nKeep assessment.druaiconsulting.com as the CTA in both versions.'}`);
    res.status(202).json({success:true,agent:route.agent_name,csq_id:id});}
