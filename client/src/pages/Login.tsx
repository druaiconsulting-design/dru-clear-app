      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        {mode === "choose" && (
          <div style={{ animation: "fadeSlideIn 0.5s ease both" }}>
            ...
            <button onClick={() => setMode("client-email")} style={{ width: "100%", background: "rgba(212,175,55,0.06)", border: "1px solid rgba(212,175,55,0.25)", borderRadius: 10, padding: "1rem 1.25rem", cursor: "pointer", textAlign: "left" }}>
              ...
              <p style={{ fontFamily: "'Inter', sans-serif", color: "rgba(230,230,230,0.45)", fontSize: "0.72rem" }}>Use your email and password</p>
            </button>
        ...
        {mode === "client-register" && (
          <div>
            ...
            {error && <p style={{ color: "#E53935", fontFamily: "'Inter', sans-serif", fontSize: "0.78rem", marginBottom: "0.875rem" }}>{error}</p>}
            <button onClick={handleClientRegister} disabled={loading} style={btnGold}>{loading ? "Creating account..." : "Create Account and Enter Portal"}</button>
          </div>
        )}
