...
type Mode = "choose" | "admin" | "client-email" | "client-login" | "client-register";
...
export default function Login() {
  const { loginAdmin, loginClient, registerClient, checkClientExists, loginWithGoogle } = useAuth();
  ...
  const handleGoogleClick = () => {
    if (!window.google) { setError("Google Sign In is loading. Please try again."); return; }
  ...
  const handleClientContinue = async () => {
    ...
    const exists = checkClientExists(email);
    setLoading(false);
    setMode(exists ? "client-login" : "client-register");
  };
  ...
  const handleClientRegister = async () => {
    ...
    if (!/[^a-zA-Z0-9]/.test(password)) { setError("Password must include at least one special character (!@#$%)."); setLoading(false); return; }
    const result = registerClient(email, password, firstName);
    setLoading(false);
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        {mode === "choose" && (
          <div>
            ...
            {/* Google Sign In */}
            <button onClick={handleGoogleClick} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.75rem", background: "#FFFFFF", color: "#1a1a1a", border: "none", borderRadius: 8, padding: "0.85rem 1rem", fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: "0.82rem", letterSpacing: "0.04em", cursor: "pointer", marginBottom: "0.875rem", transition: "opacity 0.2s" }}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
              <button onClick={() => { setMode("admin"); setError(""); }} style={{ background: "rgba(194,24,91,0.1)", border: "1px solid rgba(194,24,91,0.35)", borderRadius: 10, padding: "1rem 1.25rem", cursor: "pointer", textAlign: "left" }}>
                ...
                <p style={{ fontFamily: "'Inter', sans-serif", color: "rgba(230,230,230,0.45)", fontSize: "0.72rem" }}>For DeAnna R. Upshaw only</p>
              </button>
              <button onClick={() => { setMode("client-email"); setError(""); }} style={{ background: "rgba(212,175,55,0.06)", border: "1px solid rgba(212,175,55,0.25)", borderRadius: 10, padding: "1rem 1.25rem", cursor: "pointer", textAlign: "left" }}>
        ...
        {mode === "admin" && (
          <div>
            <button onClick={() => { setMode("choose"); setError(""); }} style={{ background: "none", border: "none", color: "rgba(212,175,55,0.6)", fontFamily: "'Inter', sans-serif", fontSize: "0.75rem", cursor: "pointer", marginBottom: "1.5rem", padding: 0 }}>← Back</button>
            ...
            <button onClick={handleAdminLogin} disabled={loading} style={{ width: "100%", background: "#C2185B", color: "#FFFFFF", border: "none", borderRadius: 6, padding: "0.85rem", fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: "0.82rem", letterSpacing: "0.06em", cursor: "pointer" }}>
        ...
        {mode === "client-email" && (
          <div>
            <button onClick={() => { setMode("choose"); setError(""); setEmail(""); }} style={{ background: "none", border: "none", color: "rgba(212,175,55,0.6)", fontFamily: "'Inter', sans-serif", fontSize: "0.75rem", cursor: "pointer", marginBottom: "1.5rem", padding: 0 }}>← Back</button>
            ...
            <p style={{ color: "rgba(230,230,230,0.45)", fontFamily: "'Inter', sans-serif", fontSize: "0.82rem", marginBottom: "1.75rem" }}>Enter the email you used for your assessment</p>
            <input type="email" placeholder="Your email address" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleClientContinue()} style={{ ...inputStyle, marginBottom: "1.25rem" }} />
            {error && <p style={{ color: "#E53935", fontFamily: "'Inter', sans-serif", fontSize: "0.78rem", marginBottom: "0.875rem" }}>{error}</p>}
            <button onClick={handleClientContinue} disabled={loading} style={{ width: "100%", background: "#D4AF37", color: "#0A2342", border: "none", borderRadius: 6, padding: "0.85rem", fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: "0.82rem", letterSpacing: "0.06em", cursor: "pointer" }}>
        ...
        {mode === "client-register" && (
          <div>
            <button onClick={() => { setMode("client-email"); setError(""); setPassword(""); setFirstName(""); }} style={{ background: "none", border: "none", color: "rgba(212,175,55,0.6)", fontFamily: "'Inter', sans-serif", fontSize: "0.75rem", cursor: "pointer", marginBottom: "1.5rem", padding: 0 }}>← Back</button>
            <h1 style={{ fontFamily: "'Playfair Display', serif", color: "#FFFFFF", fontSize: "1.75rem", fontWeight: 700, marginBottom: "0.5rem" }}>Create Your Password</h1>
            <p style={{ color: "rgba(230,230,230,0.45)", fontFamily: "'Inter', sans-serif", fontSize: "0.82rem", marginBottom: "1.75rem" }}>Welcome! Setting up access for <span style={{ color: "#D4AF37" }}>{email}</span></p>
            ...
            {error && <p style={{ color: "#E53935", fontFamily: "'Inter', sans-serif", fontSize: "0.78rem", marginBottom: "0.875rem" }}>{error}</p>}
            <button onClick={handleClientRegister} disabled={loading} style={{ width: "100%", background: "#D4AF37", color: "#0A2342", border: "none", borderRadius: 6, padding: "0.85rem", fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: "0.82rem", letterSpacing: "0.06em", cursor: "pointer" }}>
        ...
        {mode === "client-login" && (
          <div>
            <button onClick={() => { setMode("client-email"); setError(""); setPassword(""); }} style={{ background: "none", border: "none", color: "rgba(212,175,55,0.6)", fontFamily: "'Inter', sans-serif", fontSize: "0.75rem", cursor: "pointer", marginBottom: "1.5rem", padding: 0 }}>← Back</button>
            ...
            <input type="password" placeholder="Your password" value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleClientLogin()} style={{ ...inputStyle, marginBottom: "1.25rem" }} />
            {error && <p style={{ color: "#E53935", fontFamily: "'Inter', sans-serif", fontSize: "0.78rem", marginBottom: "0.875rem" }}>{error}</p>}
            <button onClick={handleClientLogin} disabled={loading} style={{ width: "100%", background: "#D4AF37", color: "#0A2342", border: "none", borderRadius: 6, padding: "0.85rem", fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: "0.82rem", letterSpacing: "0.06em", cursor: "pointer" }}>
