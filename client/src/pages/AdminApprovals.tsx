...
type ApprovalStatus = "pending" | "approved" | "rejected" | "edited" | "ready_to_use" | "read" | "scheduled" | "posted";
...
interface Approval {
  ...
  scheduled_payload: Record<string, any> | null;
}
...
const READY_TO_USE_PLATFORMS = new Set(['Copy', 'Proposal', 'Press', 'Design', 'Course', 'Video']);

// Fixed daily posting slots for the Social Media Post scheduler — Central Time, same 12 slots every day including weekends.
const SCHEDULE_SLOTS: { label: string; hour: number; minute: number }[] = [
  { label: "9:00 AM",  hour: 9,  minute: 0  }, { label: "9:15 AM",  hour: 9,  minute: 15 },
  { label: "9:30 AM",  hour: 9,  minute: 30 }, { label: "9:45 AM",  hour: 9,  minute: 45 },
  { label: "12:00 PM", hour: 12, minute: 0  }, { label: "12:15 PM", hour: 12, minute: 15 },
  { label: "12:30 PM", hour: 12, minute: 30 }, { label: "12:45 PM", hour: 12, minute: 45 },
  { label: "4:00 PM",  hour: 16, minute: 0  }, { label: "4:15 PM",  hour: 16, minute: 15 },
  { label: "4:30 PM",  hour: 16, minute: 30 }, { label: "4:45 PM",  hour: 16, minute: 45 },
];
...
const CATEGORY_LABELS: Record<string, string> = {
  ...
  social:"Social Media Post", proposals_press:"Proposals & Press Release", email:"Email", proposal:"Proposal", content:"Content",
...
const CATEGORY_ORDER = [
  ...
  "community_connection","community_post","social","proposals_press","travis_video_production","video_script","email","proposal","grants","grant_applications","content","other",
...
export default function AdminApprovals() {
  ...
  const [schedulerOpenId, setSchedulerOpenId] = useState<string | null>(null);
  const [scheduleDateChoice, setScheduleDateChoice] = useState<Record<string, "today" | "tomorrow" | string>>({});
  const [scheduleSlotChoice, setScheduleSlotChoice] = useState<Record<string, string>>({});
  ...
  // Freezes the exact payload fireSocialPublisher would have sent right now, then saves it
  // with a future scheduled_post_at instead of firing it — the cron picks it up later and
  // sends this same frozen payload through /api/social-publisher, unchanged.
  const handleSchedule = async (id: string) => {
    const approval = approvals.find(a => a.id === id);
    if (!approval) return;
    const slotLabel = scheduleSlotChoice[id];
    const slotDef = SCHEDULE_SLOTS.find(s => s.label === slotLabel);
    if (!slotDef) return;

    const dateChoice = scheduleDateChoice[id] || "today";
    const target = new Date();
    if (dateChoice === "tomorrow") target.setDate(target.getDate() + 1);
    else if (dateChoice !== "today") {
      const [y, m, d] = dateChoice.split("-").map(Number);
      target.setFullYear(y, m - 1, d);
    }
    target.setHours(slotDef.hour, slotDef.minute, 0, 0);
    ...
    const payload = isMulti ? {
      approval_id: approval.id,
      linkedin_content: approval.linkedin_content ?? approval.output,
      facebook_content: approval.facebook_content ?? "",
      instagram_caption: approval.instagram_caption ?? "",
      video_url: media.video_url || null,
      instagram_video_url: media.instagram_video_url || null,
      image_url: media.image_url || null,
      facebook_reel_url: media.facebook_reel_url || null,
      platforms_selected: getSelectedPlatforms(id),
    } : {
      content: approval.edited_output || approval.output,
      platform: approval.platform,
      approval_id: approval.id,
    ...
    const { error } = await supabase.from("approvals").update({
      status: "scheduled",
      scheduled_post_at: target.toISOString(),
      scheduled_payload: payload,
    }).eq("id", id);

    if (!error) {
      setApprovals(prev => prev.map(a => a.id === id ? { ...a, status: "scheduled", scheduled_post_at: target.toISOString() } : a));
  ...
  // "social" carries two different kinds of cards under one category value: real postable
  // LinkedIn/Facebook/Instagram content (Darius, Nia, Yara) vs. Kwame's Proposals and Ingrid's
  // Press Releases, which share the tag for filing but never publish anywhere. Split by platform.
  const isProposalsPressCard = (a: Approval) => a.category === "social" && (a.platform === "Proposal" || a.platform === "Press");
  const isSocialMediaPostCard = (a: Approval) => a.category === "social" && !isProposalsPressCard(a);
  ...
  const filtered = (() => {
    ...
    if (activeFilter === "social") return approvals.filter(isSocialMediaPostCard);
    if (activeFilter === "proposals_press") return approvals.filter(isProposalsPressCard);
  ...
  const getCategoryCount = (cat: string) => {
    ...
    if (cat === "social") return approvals.filter(isSocialMediaPostCard).length;
    if (cat === "proposals_press") return approvals.filter(isProposalsPressCard).length;
                ...
                <div key={approval.id} style={{ borderRadius:12, overflow:"hidden", border:`1px solid ${isPostTrigger ? "rgba(194,24,91,0.35)" : isCCPost ? "rgba(45,90,142,0.35)" : isKnowledge ? "rgba(10,35,66,0.12)" : (approval.status === "pending" || isAwaitingYou) ? "rgba(212,175,55,0.25)" : "rgba(10,35,66,0.08)"}`, background: (approval.status !== "pending" && !isAwaitingYou) ? "rgba(10,35,66,0.02)" : isPostTrigger ? "rgba(194,24,91,0.03)" : isCCPost ? "rgba(45,90,142,0.03)" : isKnowledge ? "rgba(10,35,66,0.02)" : "#FFFFFF", opacity:(approval.status !== "pending" && !isAwaitingYou) ? 0.7 : 1 }}>
                  ...
                  <div style={{ background:"#071A2E", padding:"0.65rem 1rem", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap" as const, gap:"0.5rem" }}>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:"0.75rem" }}>
                      ...
                      {approval.status !== "pending" && !publishStatus[approval.id] && <span style={{ fontFamily:"'Montserrat', sans-serif", fontSize:"0.55rem", fontWeight:700, letterSpacing:"0.08em", textTransform:"uppercase" as const, color:(approval.status === "approved" || approval.status === "posted") ? "#4CAF50" : (approval.status === "needs_your_input" || approval.status === "scheduled") ? "#D4AF37" : "#C2185B" }}>{approval.status === "needs_your_input" ? "Needs Your Feedback" : approval.status === "scheduled" && approval.scheduled_post_at ? `Scheduled for ${formatScheduledTime(approval.scheduled_post_at)} CT` : approval.status === "posted" ? "✓ Posted" : approval.status}</span>}
                  ...
                  <div style={{ padding:"0 1rem 1rem", display:"flex", gap:"0.5rem", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap" as const }}>
                    ...
                    <div style={{ display:"flex", gap:"0.5rem", marginLeft:"auto" }}>
                      ...
                      {!isKnowledge && approval.status === "pending" && editingId !== approval.id && rejectingId !== approval.id && !isPostTrigger && (
                        <>
                          ...
                          {/* Approve + Schedule — Social Media Post cards only (Darius/Nia/Yara), not Proposals & Press Release */}
                          {isSocial && !isReadyToUse && approval.status === 'pending' && approval.category === 'social' && (
                            <button
                              onClick={() => setSchedulerOpenId(schedulerOpenId === approval.id ? null : approval.id)}
                              disabled={isMulti && getSelectedPlatforms(approval.id).length === 0}
                              style={{ fontFamily:"'Montserrat', sans-serif", fontSize:"0.62rem", fontWeight:700, padding:"0.45rem 1.25rem", borderRadius:6, cursor:"pointer", border:"1px solid #D4AF37", background: schedulerOpenId === approval.id ? "#D4AF37" : "transparent", color: schedulerOpenId === approval.id ? "#0A2342" : "#D4AF37", letterSpacing:"0.06em", opacity:(isMulti && getSelectedPlatforms(approval.id).length === 0) ? 0.5 : 1 }}>
                              Approve + Schedule {schedulerOpenId === approval.id ? "▲" : "▾"}
                            </button>
                          )}
                    ...
                    {/* Slot picker — appears under the buttons when "Approve + Schedule" is toggled open */}
                    {schedulerOpenId === approval.id && (
                      <div style={{ marginTop:"0.75rem", padding:"0.85rem", borderRadius:8, background:"rgba(212,175,55,0.06)", border:"1px solid rgba(212,175,55,0.25)" }}>
                        <div style={{ display:"flex", gap:"0.5rem", marginBottom:"0.65rem", alignItems:"center", flexWrap:"wrap" as const }}>
                          {(["today", "tomorrow"] as const).map(d => (
                            <button key={d} onClick={() => setScheduleDateChoice(prev => ({ ...prev, [approval.id]: d }))}
                              style={{ fontFamily:"'Montserrat', sans-serif", fontSize:"0.6rem", fontWeight:700, padding:"0.35rem 0.9rem", borderRadius:6, cursor:"pointer", border:"1px solid rgba(10,35,66,0.25)", background:(scheduleDateChoice[approval.id] ?? "today") === d ? "#0A2342" : "transparent", color:(scheduleDateChoice[approval.id] ?? "today") === d ? "#FAFAF8" : "#0A2342", letterSpacing:"0.06em", textTransform:"capitalize" as const }}>
                              {d}
                            </button>
                          ))}
                          <input type="date" onChange={e => setScheduleDateChoice(prev => ({ ...prev, [approval.id]: e.target.value }))}
                            value={!["today","tomorrow"].includes(scheduleDateChoice[approval.id] ?? "today") ? (scheduleDateChoice[approval.id] as string) : ""}
                            style={{ fontFamily:"'Montserrat', sans-serif", fontSize:"0.6rem", padding:"0.3rem 0.5rem", borderRadius:6, border:"1px solid rgba(10,35,66,0.25)", background:"#FFFFFF", color:"#0A2342" }} />
                        </div>
                        <div style={{ display:"flex", gap:"0.4rem", flexWrap:"wrap" as const, marginBottom:"0.75rem" }}>
                          {SCHEDULE_SLOTS.map(slot => (
                            <button key={slot.label} onClick={() => setScheduleSlotChoice(prev => ({ ...prev, [approval.id]: slot.label }))}
                              style={{ fontFamily:"'Montserrat', sans-serif", fontSize:"0.58rem", fontWeight:700, padding:"0.35rem 0.6rem", borderRadius:6, cursor:"pointer", border:"1px solid rgba(212,175,55,0.5)", background:scheduleSlotChoice[approval.id] === slot.label ? "#D4AF37" : "transparent", color:scheduleSlotChoice[approval.id] === slot.label ? "#0A2342" : "#D4AF37", letterSpacing:"0.04em" }}>
                              {slot.label}
                            </button>
                          ))}
                        </div>
                        <div style={{ display:"flex", gap:"0.5rem" }}>
                          <button onClick={() => handleSchedule(approval.id)} disabled={scheduling === approval.id || !scheduleSlotChoice[approval.id]}
                            style={{ fontFamily:"'Montserrat', sans-serif", fontSize:"0.62rem", fontWeight:700, padding:"0.45rem 1.25rem", borderRadius:6, cursor:"pointer", border:"none", background:"#D4AF37", color:"#0A2342", letterSpacing:"0.06em", opacity:(scheduling === approval.id || !scheduleSlotChoice[approval.id]) ? 0.5 : 1 }}>
                            {scheduling === approval.id ? "Scheduling..." : "Confirm Schedule"}
                          </button>
                          <button onClick={() => setSchedulerOpenId(null)}
