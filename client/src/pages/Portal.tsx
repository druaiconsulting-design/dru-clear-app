...
function PortalAvatar({ user }: { user: any }) {
  ...
  return (
    <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(212,175,55,0.1)", border: "2px solid rgba(212,175,55,0.35)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
      <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "1.1rem", fontWeight: 700, color: "#D4AF37", lineHeight: 1 }}>{initials}</span>
    </div>
  );
}

// ── Support Modal — stays dark as overlay ─────────────────────────────────────
    ...
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: "1.5rem" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#0d2340", border: "1px solid rgba(212,175,55,0.3)", borderRadius: 12, padding: "2rem 1.75rem", maxWidth: 360, width: "100%", boxShadow: "0 24px 60px rgba(0,0,0,0.6)" }}>
        ...
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(212,175,55,0.15)", borderRadius: 8, padding: "0.75rem 1rem", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "0.75rem" }}>
          ...
          <button onClick={handleCopy} style={{ background: "none", border: "none", color: copied ? "#43A047" : "rgba(212,175,55,0.7)", fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: "0.65rem", letterSpacing: "0.06em", cursor: "pointer", whiteSpace: "nowrap", padding: 0, transition: "color 0.2s" }}>
            {copied ? "✓ Copied" : "Copy"}
          </button>
        </div>
      </div>
...
function getStageStyle(stageName: string, pathwayStage: PathwayStage): React.CSSProperties {
  ...
  if (isCurrent) return { background: "#C2185B", border: "1px solid #C2185B", borderRadius: 6, padding: "0.4rem 0.75rem", textAlign: "center" as const };
  if (active)    return { background: "rgba(212,175,55,0.12)", border: "1px solid rgba(212,175,55,0.4)", borderRadius: 6, padding: "0.4rem 0.75rem", textAlign: "center" as const };
  return          { background: "rgba(10,35,66,0.04)", border: "1px solid rgba(10,35,66,0.12)", borderRadius: 6, padding: "0.4rem 0.75rem", textAlign: "center" as const };
}
...
function getStageTextStyle(stageName: string, pathwayStage: PathwayStage): React.CSSProperties {
  const isCurrent =
    ...
    ((stageName === "Deploy" || stageName === "Dominate") && pathwayStage === "deploy");
  const active = isStageActive(stageName, pathwayStage);
  return { fontFamily: "'Montserrat', sans-serif", color: isCurrent ? "#FFFFFF" : active ? "#D4AF37" : "rgba(10,35,66,0.3)", fontWeight: 700, fontSize: "0.72rem", letterSpacing: "0.06em" };
}
...
export default function Portal() {
  ...
  useEffect(() => { if (user?.id) checkDailyStatus(); }, [user?.id]); // eslint-disable-line
  ...
  const getDailyBorder = () => dailyState === "completed" && currentStreak >= 7 ? "1px solid rgba(212,175,55,0.6)" : "1px solid rgba(10,35,66,0.1)";
  const getDailyGlow   = () => dailyState === "completed" && currentStreak >= 7 ? "0 0 18px rgba(212,175,55,0.2)" : "none";
    ...
    <>
      ...
      <AdminLayout currentPath={window.location.pathname}>
        <main style={{ flex: 1, padding: isMobile() ? "1.25rem 1rem" : "2.5rem 1.5rem", maxWidth: 680, margin: "0 auto", width: "100%" }}>
          ...
          <div style={{ marginBottom: "2rem" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "0.875rem" }}>
              ...
              <div>
                ...
                {user?.email && <p style={{ fontFamily: "'Inter', sans-serif", color: "rgba(10,35,66,0.35)", fontSize: "0.72rem", margin: 0 }}>{user.email}</p>}
              </div>
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile() ? "1fr" : "1fr 1fr 1fr", gap: "0.75rem", marginBottom: "1.25rem" }}>
            {QUICK_ACTIONS.map((item) => {
              ...
              const cardStyle: React.CSSProperties = {
                background: "#FFFFFF",
                border: isDaily ? getDailyBorder() : "1px solid rgba(10,35,66,0.1)",
              ...
              const inner = (
                <div style={cardStyle}
                  onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(212,175,55,0.5)"; (e.currentTarget as HTMLDivElement).style.background = "rgba(212,175,55,0.04)"; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.borderColor = isDaily ? getDailyBorder() : "rgba(10,35,66,0.1)"; (e.currentTarget as HTMLDivElement).style.background = "#FFFFFF"; }}>
              ...
              if (item.onClick) return <div key={item.key} style={{ textDecoration: "none" }} onClick={item.onClick}>{inner}</div>;
          ...
          {/* Passkey card */}
          {!passkeyDismissed && (
            <div style={{ background: hasPasskey ? "rgba(67,160,71,0.05)" : "#FFFFFF", border: hasPasskey ? "1px solid rgba(67,160,71,0.25)" : "1px solid rgba(10,35,66,0.1)", borderRadius: 10, padding: "1rem 1.25rem", marginBottom: "1.25rem", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "1rem" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.85rem", flex: 1, minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <p style={{ fontFamily: "'Montserrat', sans-serif", color: hasPasskey ? "#43A047" : "#0A2342", fontWeight: 700, fontSize: "0.78rem", letterSpacing: "0.04em", margin: 0, marginBottom: "0.1rem" }}>
                    {hasPasskey ? "Passkey Active" : "Speed Up Your Login"}
                  </p>
                  <p style={{ fontFamily: "'Inter', sans-serif", color: "rgba(10,35,66,0.45)", fontSize: "0.68rem", margin: 0, lineHeight: 1.4 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexShrink: 0 }}>
                {!hasPasskey && (
                  <button onClick={handleSetupPasskey} disabled={passkeyLoading}
                    style={{ background: "#D4AF37", color: "#0A2342", border: "none", borderRadius: 6, padding: "0.55rem 1rem", fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: "0.7rem", letterSpacing: "0.06em", cursor: passkeyLoading ? "default" : "pointer", opacity: passkeyLoading ? 0.7 : 1, whiteSpace: "nowrap" }}>
          ...
          {dailyState === "completed" && currentStreak >= 7 && (
            <div style={{ background: "rgba(212,175,55,0.05)", border: "1px solid rgba(212,175,55,0.25)", borderRadius: 8, padding: "0.75rem 1rem", marginBottom: "1.5rem", display: "flex", alignItems: "center", gap: "0.75rem" }}>
              ...
              <div>
                ...
                <p style={{ fontFamily: "'Inter', sans-serif", color: "rgba(10,35,66,0.45)", fontSize: "0.65rem", margin: 0, marginTop: 2 }}>Consistency is the compounding advantage most leaders never unlock.</p>
              </div>
            </div>
          )}

          {/* Transformation Pathway */}
          <div style={{ background: "#FFFFFF", border: "1px solid rgba(10,35,66,0.1)", borderRadius: 10, padding: "1.5rem", marginBottom: "1.5rem" }}>
            ...
            <div style={{ display: "flex", gap: "0.5rem", alignItems: "center", overflowX: "auto", paddingBottom: "0.5rem" }}>
              {PATHWAY_STAGES.map((stage, i) => (
                <div key={stage} style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexShrink: 0 }}>
                  ...
                  {i < 4 && <span style={{ color: isStageActive(PATHWAY_STAGES[i + 1], pathwayStage) ? "rgba(212,175,55,0.6)" : "rgba(10,35,66,0.15)", fontSize: "0.8rem" }}>→</span>}
                </div>
              ))}
            </div>
            <p style={{ fontFamily: "'Inter', sans-serif", color: "rgba(10,35,66,0.45)", fontSize: "0.7rem", marginTop: "0.75rem", fontStyle: "italic" }}>{getStatusText(pathwayStage)}</p>
          </div>
