 ...
 * screens/ShareYourExcitement.tsx
 */

import { useState, useRef } from "react";
import { LeadData, Scores, BADGE_URLS } from "../types";
import { getTier, sendWebhook, UTM_PARAMS } from "../Utils";

export function ShareYourExcitementScreen({
  lead, scores, onRevisit,
}: {
  lead: LeadData;
  scores: Scores;
  onRevisit: () => void;
  ...
  const shareBtnStyle = (color: string): React.CSSProperties => ({
    ...
    width: "100%", padding: "0.75rem 1rem", background: `${color}14`, color: color,
    border: `1.5px solid ${color}50`, borderRadius: 6, fontFamily: "'Montserrat', sans-serif",
  ...
  return (
    <div
      ...
      style={{ minHeight: "100dvh", background: "#0A2342", padding: "2rem 1.5rem 3rem", textAlign: "center" }}
    >
      {/* Header */}
      <div className="flex justify-between items-center w-full mb-4" style={{ maxWidth: 360 }}>
        <img src="https://files.manuscdn.com/user_upload_by_module/session_file/310519663512997684/NJTJspnSktvZQJaw.png" alt="DRU CLEAR™ Logo" style={{ height: 80, width: "auto", objectFit: "contain" }} />
      ...
      <h2 className="text-3xl font-bold mb-3" style={{ fontFamily: "'Playfair Display', serif", color: "#D4AF37", lineHeight: 1.2, maxWidth: 340 }}>
      ...
      {badgeUrl && (
        <div className="flex flex-col items-center" style={{ gap: "0.4rem", marginBottom: "1.5rem", width: "100%", maxWidth: 340, position: "relative", zIndex: 1 }}>
          <button
            onClick={async () => {
              try { const res = await fetch(badgeUrl); const blob = await res.blob(); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `DRU-CLEAR-Badge-${tier.label}.png`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); }
        ...
        <div className="flex flex-col gap-2 mb-3">
          ...
          <button onClick={handleCopyLink} style={shareBtnStyle("#FFFFFF")}>
            {copied
              ...
              : <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3" /><circle cx="6" cy="12" r="3" /><circle cx="18" cy="19" r="3" /><line x1="8.59" y1="13.51" x2="15.42" y2="17.49" /><line x1="15.41" y1="6.51" x2="8.59" y2="10.49" /></svg>Copy My Referral Link</>
            }
          </button>
        </div>
      ...
      <div style={{ width: "100%", maxWidth: 340, marginBottom: "1.5rem", textAlign: "left" }}>
        <p style={{ color: "#D4AF37", fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: "0.72rem", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "0.35rem" }}>
          READY-MADE COPY <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0, fontSize: "0.63rem", color: "rgba(230,230,230,0.45)" }}>(Tap to expand)</span>
        </p>
        ...
        {[
          { id: "linkedin", label: "LinkedIn", color: "#0A66C2", caption: linkedInCaption },
          { id: "whatsapp", label: "WhatsApp", color: "#25D366", caption: whatsAppCaption },
        ].map((ch) => (
          <div key={ch.id} style={{ marginBottom: "0.65rem" }}>
            <button
              onClick={() => setExpandedCaption(expandedCaption === ch.id ? null : ch.id)}
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", width: "100%", padding: "0.55rem 0.75rem", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(212,175,55,0.15)", borderRadius: expandedCaption === ch.id ? "6px 6px 0 0" : 6, color: "#E6E6E6", fontFamily: "'Montserrat', sans-serif", fontWeight: 600, fontSize: "0.75rem", cursor: "pointer" }}
            >
        ...
        {!colleagueSent ? (
          <>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              <input
                className="dru-input" type="email" placeholder="colleague@company.com"
              ...
              <button
                ...
                style={{ padding: "0 1rem", background: "#D4AF37", color: "#0A2342", border: "none", borderRadius: 4, fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: "0.78rem", cursor: "pointer", whiteSpace: "nowrap" }}
              >
                Send →
              </button>
            </div>
      ...
      <div style={{ width: "100%", maxWidth: 340, marginBottom: "1.5rem", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: "0.75rem", justifyContent: "center" }}>
          <button
            onClick={() => handleFeedback("up")} disabled={!!feedback}
            style={{ display: "flex", alignItems: "center", gap: "0.4rem", padding: "0.55rem 1.25rem", background: feedback === "up" ? "rgba(67,160,71,0.15)" : "rgba(255,255,255,0.05)", border: `1.5px solid ${feedback === "up" ? "#43A047" : "rgba(255,255,255,0.15)"}`, borderRadius: 6, color: feedback === "up" ? "#43A047" : "#E6E6E6", fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: "0.8rem", cursor: feedback ? "default" : "pointer", transition: "all 0.2s" }}
          >👍 Yes</button>
          <button
            onClick={() => handleFeedback("down")} disabled={!!feedback}
            style={{ display: "flex", alignItems: "center", gap: "0.4rem", padding: "0.55rem 1.25rem", background: feedback === "down" ? "rgba(229,57,53,0.1)" : "rgba(255,255,255,0.05)", border: `1.5px solid ${feedback === "down" ? "#E53935" : "rgba(255,255,255,0.15)"}`, borderRadius: 6, color: feedback === "down" ? "#E53935" : "#E6E6E6", fontFamily: "'Montserrat', sans-serif", fontWeight: 700, fontSize: "0.8rem", cursor: feedback ? "default" : "pointer", transition: "all 0.2s" }}
          >👎 Not Really</button>
        </div>
        {feedback && (
      ...
      <div style={{ width: "100%", maxWidth: 340, marginBottom: "1.75rem", textAlign: "center" }}>
        <button
          onClick={onRevisit}
          style={{ background: "none", border: "none", color: "#D4AF37", fontWeight: 700, fontSize: "0.82rem", textDecoration: "underline", textUnderlineOffset: 3, cursor: "pointer", fontFamily: "'Montserrat', sans-serif" }}
        >
          → Revisit Your Diagnostic Options
        </button>
      </div>
